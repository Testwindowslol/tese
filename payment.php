<?php
  header("Location: https://t.me/HexstresserV2");
  exit();
?>
...
$paginaname = 'Bitcoin';
	...
	{
	$id = (int)$_GET['id'];
	$row = $odb -> query("SELECT * FROM `addons` WHERE `ID` = '$id'") -> fetch();
	if($row == "")
	{
	 header("Location: index.php");
	} else {
      ...
      <div class="card-body">
        <div class="container-fluid d-flex justify-content-between">
          ...
          <div class="col-lg-3 pr-0">
            <h4 class="font-weight-medium text-uppercase text-right mt-4 mb-2">Addon</h4>
            <h4 class="text-right mb-5 pb-4"><?php echo $row['addon']; ?></h4>
          </div>
        </div>
