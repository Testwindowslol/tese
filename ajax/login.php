...
if ($type == 'login') {
    ...
    // Log IP address
    $stmt = $odb->prepare("INSERT INTO ip_logs(ip, ip_hash, user_id) VALUES(?, ?, ?)");
    $stmt->execute([
        encrypt($ip, getStaticKey("ip_logs")),
        sha1(getStaticKey("ip_logs") . ":" . $ip),
        $userInfo['ID']
    ]);
...
if ($type == 'register') {
    ...
    if ($ipLogsCount > 0) {
        die(error(' You already have an account'));
    }
