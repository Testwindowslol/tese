...
function popup() {
    ...
    var url = "ping.php?host=" + host + "&time=" + time;
    var popup = window.open(url, "Ping Popup", "width=400,height=400");
    setTimeout(function(){ popup.close(); }, time*1000);
...
function start() {
    ...
    xmlhttp.open("GET", "ajax/hub.php?type=start" + "&host=" + host + "&port=" + port + "&time=" + time + "&method=" + method + "&concurrents=" + concurrents, true);
    xmlhttp.send();
}
function renew(id) {
    rere.disabled = true;
