<?php
include("@/header.php");
$paginaname = 'Payment Invoice';

ob_start();
session_start();
require_once("@/config.php");
require_once("@/init.php");

if ($_GET['id'] == "") {
    header("Location: index.php");
} else {
    if (isset($_GET['id']) && is_numeric($_GET['id']) && $user->LoggedIn()) {
        $id = (int)$_GET['id'];

        // Vérifier si le plan est privé
        $SQLGetPlans = $odb->query("SELECT * FROM `plans` WHERE `ID` = '$id' AND `private` = 0");
        $row = $SQLGetPlans->fetch();
        if ($row == "") {

        } else {
            // Récupérer le solde de l'utilisateur
            $user_id = $_SESSION['ID'];
            $selectBalanceQuery = "SELECT credits FROM user_balance WHERE user_id = :user_id";
            $stmt = $odb->prepare($selectBalanceQuery);
            $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
            $stmt->execute();
            $balance = $stmt->fetchColumn();

            // Vérifier si l'utilisateur a déjà ce plan
            $checkUserPlanQuery = "SELECT membership FROM users WHERE ID = :user_id";
            $stmt = $odb->prepare($checkUserPlanQuery);
            $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
            $stmt->execute();
            $userMembership = $stmt->fetchColumn();

            // Vérifier si l'utilisateur a suffisamment de crédits pour effectuer le paiement
            $price = $row['price'];
            $canPay = $balance >= $price;
        }
    }
}
?>

<!DOCTYPE html>
<!--[if IE 9]>         <html class="no-js lt-ie10"> <![endif]-->
<!--[if gt IE 9]><!--> <html class="no-js"> <!--<![endif]-->
<head>
  <script src="../assets/js/spinner.js"></script>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css">

<div class="page-wrapper">
     <div class="page-content">
    <div class="alert alert-fill-primary">    
      <span data-feather="credit-card" class="icon-md text-light mr-2"></span>
   <span><?php echo htmlspecialchars($paginaname); ?></span>
   </div>
<div class="row">
  <div class="col-md-12">
    <div class="card">
      <div class="card-body">
        <div class="container-fluid d-flex justify-content-between">
          <div class="col-lg-3 pl-0">
            <a href="#" class="noble-ui-logo logo-light d-block mt-3"><?php echo ($sitename)?></a>                 
            <h5 class="mt-5 mb-2 text-muted">Invoice to : <?php echo $_SESSION['username']?></h5>
          </div>
          <div class="col-lg-3 pr-0">
            <h4 class "font-weight-medium text-uppercase text-right mt-4 mb-2">Package</h4>
            <h4 class="text-right mb-5 pb-4"><?php echo $row['name']; ?></h4>
          </div>
        </div>
        <div class="container-fluid mt-5 d-flex justify-content-center w-100">
          <div class="table-responsive w-100">
              <table class="table table-bordered">
                <thead>
                  <tr>
                      <th>Attack Time</th>
                      <th class="text-right">Concurrents</th>
                      <th class="text-right">Length</th>
                      <th class="text-right">Price</th>
                    </tr>
                </thead>
                <tbody>
                  <tr class="text-right">                    
                    <td class="text-left"><?php echo $row['mbt']; ?></td>
                    <td><?php echo $row['concurrents']; ?></td>
                    <td><?php echo $row['length']; ?> <?php echo $row['unit']; ?></td>
                    <td>$<?php echo $row['price']; ?></td>
                  </tr>
                </tbody>
              </table>
            </div>
        </div>
        <div class="container-fluid mt-5 w-100">
          <div class="row">
            <div class="col-md-12 ml-auto">
              
            <form action="process_payment.php" method="POST">
                <p> <i class='fas fa-coins'></i> Your balance: <span style="color: #6412a3;"> $<?php echo number_format($balance, 2); ?></span></p>
                <?php
                if ($userMembership == $id) {
                    echo '<div class="alert alert-danger">You already have these plans.</div>';
                } elseif ($canPay) {
                    echo '<button type="submit" class="btn btn-primary" name="payButton" value="' . $id . '">Pay Now!</button>';
                } else {
                    echo '<div class="alert alert-danger">You do not have enough credits in your balance to make this payment.</div>';
                }
                ?>
            </form>
            </div>
          </div>
        </div>
        </div>
      </div>
    </div>
</div>
</div>
</div>
<?php include("@/script.php"); ?>
</body>
</html>