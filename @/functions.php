...
class user
{
	...
	function isPremium($odb)
	{
		...
		*/
		...
		$membership = $SQL->fetchColumn(0);

		//echo "membership :";
		//echo $membership;

		$SQL = $odb->prepare("SELECT `premium` FROM `plans` WHERE `ID` = :membership");
		$SQL->execute(array(
			":membership" => $membership
		));
		$premium = $SQL->fetchColumn(0);
...
class stats
{
	...
	function paidUsers($odb)
	{
		$SQL = $odb -> query("SELECT COUNT(*) FROM `users` WHERE `expire` > UNIX_TIMESTAMP() AND `membership`!=140");
		return $SQL->fetchColumn(0);
	}
	...
	function recentAttacks($odb)
	{
		$idToSearch = 1;
	
		$getCurrentCountSQL = "SELECT count FROM recent_attacks WHERE id = :idToSearch";
		$stmt = $odb->prepare($getCurrentCountSQL);
		$stmt->bindParam(':idToSearch', $idToSearch, PDO::PARAM_INT);
		$stmt->execute();
